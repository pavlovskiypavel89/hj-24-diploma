...
const initWSSConnection = id => {
  ...
  socket.addEventListener("close", event => console.log(event.wasClean ? `${event.reason} ${event.reason ? event.reason : '"Чистое закрытие" соединения'}` : `Обрыв связи. Причина: ${event.reason}`));
